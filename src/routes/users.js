...
function getUserOr404(req, res) {
  const user = db.prepare('SELECT id, name, email, role, active, hourly_rate FROM users WHERE id = ?').get(req.params.id);
  if (!user) {
...
router.get('/', (req, res) => {
  const users = db.prepare('SELECT id, name, email, role, active FROM users WHERE active = 1 ORDER BY role, name').all();
...
router.post('/', verifyCsrf, (req, res) => {
  ...
  const nextSortOrder = db.prepare('SELECT COALESCE(MAX(sort_order), 0) + 1 AS next FROM users').get().next;

  db.prepare('INSERT INTO users (name, email, password_hash, role, sort_order, hourly_rate) VALUES (?, ?, ?, ?, ?, ?)').run(
    b.name.trim(),
    email,
...
router.get('/:id/edit', (req, res) => {
  const targetUser = getUserOr404(req, res);
...
router.post('/:id', verifyCsrf, (req, res) => {
  ...
  db.prepare(
    `UPDATE users SET name = ?, email = ?, role = ?, active = ?, hourly_rate = ?, updated_at = datetime('now') WHERE id = ?`
  ).run(
    b.name.trim(),
  ...
  if (b.password) {
    ...
    db.prepare('UPDATE users SET password_hash = ? WHERE id = ?').run(passwords.hash(b.password), targetUser.id);
  }
...
router.post('/:id/delete', verifyCsrf, (req, res) => {
  ...
  db.prepare("UPDATE users SET active = 0, updated_at = datetime('now') WHERE id = ?").run(targetUser.id);
