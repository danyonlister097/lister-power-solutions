...
router.get('/me', (req, res) => {
  const lastEvent = getLastEvent(req.user.id);
  const isClockedIn = Boolean(lastEvent && lastEvent.type === 'in');
  const recent = db.prepare('SELECT * FROM clock_events WHERE user_id = ? ORDER BY occurred_at DESC LIMIT 20').all(req.user.id);
...
router.post('/toggle', verifyCsrf, (req, res) => {
  ...
  db.prepare(
    'INSERT INTO clock_events (user_id, type, occurred_at, latitude, longitude, accuracy) VALUES (?, ?, ?, ?, ?, ?)'
...
router.get('/team', requireRole('admin'), (req, res) => {
  const dayIso = /^\d{4}-\d{2}-\d{2}$/.test(req.query.date || '') ? req.query.date : toIsoDate(new Date());

  const users = db.prepare('SELECT id, name, sort_order FROM users WHERE active = 1 ORDER BY sort_order, name').all();
  const events = db
    .prepare('SELECT * FROM clock_events WHERE date(occurred_at) = date(?) ORDER BY user_id, occurred_at ASC')
    .all(dayIso);
...
router.get('/timesheets', requireRole('admin'), (req, res) => {
  const startIso = /^\d{4}-\d{2}-\d{2}$/.test(req.query.start || '')
    ? req.query.start
  ...
  const users = db.prepare('SELECT id, name, sort_order FROM users WHERE active = 1 ORDER BY sort_order, name').all();
  const events = db
    .prepare('SELECT * FROM clock_events WHERE date(occurred_at) BETWEEN date(?) AND date(?) ORDER BY user_id, occurred_at ASC')
    .all(startIso, endIso);
...
router.get('/edit/:userId/:date', requireRole('admin'), (req, res) => {
  ...
  const events = db
    .prepare('SELECT * FROM clock_events WHERE user_id = ? AND date(occurred_at) = date(?) ORDER BY occurred_at ASC')
    .all(user.id, dateIso);
...
router.post('/edit/:userId/:date/events', requireRole('admin'), verifyCsrf, (req, res) => {
  ...
  if (!time) {
    ...
    db.prepare('INSERT INTO clock_events (user_id, type, occurred_at) VALUES (?, ?, ?)').run(
      req.params.userId,
      type,
      `${dateIso}T${time}:00`
    );
...
router.post('/events/:id', requireRole('admin'), verifyCsrf, (req, res) => {
  const event = db.prepare('SELECT * FROM clock_events WHERE id = ?').get(req.params.id);
  ...
  if (!time) {
    ...
    db.prepare('UPDATE clock_events SET type = ?, occurred_at = ? WHERE id = ?').run(type, `${dateIso}T${time}:00`, event.id);
    setFlash(req, 'success', 'Clock event updated.');
  }
  res.redirect(`/timeclock/edit/${event.user_id}/${dateIso}`);
});

router.post('/events/:id/delete', requireRole('admin'), verifyCsrf, (req, res) => {
  const event = db.prepare('SELECT * FROM clock_events WHERE id = ?').get(req.params.id);
  if (!event) return res.status(404).render('error', { message: 'Clock event not found.' });

  const dateIso = event.occurred_at.slice(0, 10);
  const userId = event.user_id;
  db.prepare('DELETE FROM clock_events WHERE id = ?').run(event.id);
