...
router.get('/', (req, res) => {
  ...
  const customers = q
    ? db
        .prepare('SELECT * FROM customers WHERE active = 1 AND name LIKE ? ORDER BY name')
        .all(`%${q}%`)
...
router.get('/:id', (req, res) => {
  const customer = getCustomerOr404(req, res);
  if (!customer) return;
  const jobs = db
...
router.post('/:id/assets', verifyCsrf, (req, res) => {
  const customer = getCustomerOr404(req, res);
  if (!customer) return;
...
router.get('/:id/assets/:assetId', (req, res) => {
  ...
  const asset = db.prepare('SELECT * FROM customer_assets WHERE id = ? AND customer_id = ?').get(req.params.assetId, customer.id);
...
router.post('/:id/assets/:assetId', verifyCsrf, (req, res) => {
  const customer = getCustomerOr404(req, res);
  if (!customer) return;
...
router.post('/:id/assets/:assetId/delete', verifyCsrf, (req, res) => {
  const customer = getCustomerOr404(req, res);
  if (!customer) return;
  ...
  db.prepare('DELETE FROM job_assets WHERE asset_id = ?').run(asset.id);
  db.prepare('DELETE FROM customer_assets WHERE id = ?').run(asset.id);
...
router.post('/:id/delete', verifyCsrf, (req, res) => {
  const customer = getCustomerOr404(req, res);
  if (!customer) return;
  db.prepare("UPDATE customers SET active = 0, updated_at = datetime('now') WHERE id = ?").run(customer.id);
