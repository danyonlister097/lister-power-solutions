...
function getCategories() {
  return db
    .prepare('SELECT DISTINCT category FROM inventory_items WHERE category IS NOT NULL ORDER BY category')
    .all()
    .map((r) => r.category);
}
...
router.get('/', (req, res) => {
  ...
  res.render('inventory/index', { title: 'Inventory', items, categories: getCategories() });
});
...
router.post('/', requireRole('admin'), verifyCsrf, (req, res) => {
  ...
  db.prepare(
    ...
    name,
    resolveCategory(req.body, getCategories()),
...
router.post('/bulk-delete', requireRole('admin'), verifyCsrf, (req, res) => {
  const ids = []
  ...
  const inUseIds = new Set(
    db
      .prepare(`SELECT DISTINCT item_id FROM job_stock_allocations WHERE item_id IN (${placeholders})`)
      .all(...ids)
      .map((r) => r.item_id)
  );
  ...
  if (deletableIds.length) {
    ...
    db.prepare(`DELETE FROM inventory_items WHERE id IN (${delPlaceholders})`).run(...deletableIds);
  }
...
// as an item id.
router.post('/allocate', verifyCsrf, (req, res) => {
  const job = db.prepare('SELECT id, title FROM jobs WHERE id = ?').get(req.body.job_id);
  const item = db.prepare('SELECT * FROM inventory_items WHERE id = ?').get(req.body.item_id);
  ...
  db.prepare(`UPDATE inventory_items SET quantity_on_hand = quantity_on_hand - ?, updated_at = datetime('now') WHERE id = ?`).run(
    quantity,
    item.id
  );
  ...
  db.prepare(
    `INSERT INTO job_stock_allocations (job_id, item_id, quantity, cost_item_id, allocated_by) VALUES (?, ?, ?, ?, ?)`
...
// fix it themselves.
router.post('/allocations/:id/delete', verifyCsrf, (req, res) => {
  const allocation = db.prepare('SELECT * FROM job_stock_allocations WHERE id = ?').get(req.params.id);
  ...
  db.prepare('DELETE FROM job_stock_allocations WHERE id = ?').run(allocation.id);

  if (allocation.cost_item_id) {
    db.prepare('DELETE FROM job_cost_items WHERE id = ?').run(allocation.cost_item_id);
  }
...
router.post('/:id/adjust', requireRole('admin'), verifyCsrf, (req, res) => {
  const item = db.prepare('SELECT * FROM inventory_items WHERE id = ?').get(req.params.id);
  if (!item) return res.status(404).render('error', { message: 'Item not found.' });
...
router.post('/:id/delete', requireRole('admin'), verifyCsrf, (req, res) => {
  ...
  const inUse = db.prepare('SELECT COUNT(*) AS n FROM job_stock_allocations WHERE item_id = ?').get(item.id);
  if (inUse.n > 0) {
