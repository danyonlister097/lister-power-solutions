...
router.get('/', (req, res) => {
  const users = db.prepare('SELECT id, name FROM users WHERE active = 1 ORDER BY sort_order, name').all();
  const open = db
    .prepare(
...
router.post('/', verifyCsrf, (req, res) => {
  ...
  db.prepare('INSERT INTO tasks (title, assigned_to, created_by) VALUES (?, ?, ?)').run(
    title,
    assignedTo,
    req.user.id
  );
...
router.post('/:id/toggle', verifyCsrf, (req, res) => {
