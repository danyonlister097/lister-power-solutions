...
router.get('/', requireRole('admin'), (req, res) => {
  ...
  const jobsThisWeek = db
    .prepare('SELECT COUNT(*) AS n FROM jobs WHERE date(scheduled_start) BETWEEN date(?) AND date(?)')
    .get(weekStartIso, weekEndIso).n;

  const jobsToday = db
    .prepare('SELECT COUNT(*) AS n FROM jobs WHERE date(scheduled_start) = date(?)')
    .get(todayIso).n;
  ...
  const revenueThisMonth = db
    .prepare(
      `SELECT COALESCE(SUM(quantity * unit_price), 0) AS total
       ...
       WHERE invoices.status = 'paid' AND date(invoices.paid_at) >= date(?)`
    )
  ...
  const weekJobs = db
    .prepare('SELECT id, scheduled_start, scheduled_end FROM jobs WHERE date(scheduled_start) BETWEEN date(?) AND date(?)')
  ...
  if (jobIds.length) {
    ...
    const assignees = db
      .prepare(`SELECT job_id, user_id FROM job_assignees WHERE job_id IN (${placeholders})`)
      .all(...jobIds);
