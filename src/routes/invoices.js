...
router.get('/', (req, res) => {
  ...
  `;
  let invoices = db.prepare(sql).all().map((inv) => ({ ...inv, effective_status: effectiveStatus(inv) }));
...
router.post('/', verifyCsrf, (req, res) => {
  const job = db.prepare('SELECT * FROM jobs WHERE id = ?').get(req.body.job_id);
  if (!job) {
  ...
  const result = db
    .prepare('INSERT INTO invoices (job_id, invoice_number, issue_date, due_date, created_by) VALUES (?, ?, ?, ?, ?)')
    .run(job.id, nextInvoiceNumber(), today, dueDate, req.user.id);
  ...
  costItems.forEach((i) => {
    insertItem.run(result.lastInsertRowid, i.category, i.description, i.quantity, i.unit_cost);
  });
...
router.post('/:id', verifyCsrf, (req, res) => {
  ...
  db.prepare(
    "UPDATE invoices SET issue_date = ?, due_date = ?, notes = ?, updated_at = datetime('now') WHERE id = ?"
...
router.post('/:id/send', verifyCsrf, (req, res) => {
  ...
  db.prepare("UPDATE invoices SET status = 'sent', updated_at = datetime('now') WHERE id = ?").run(invoice.id);
  setFlash(req, 'success', 'Invoice marked as sent.');
  res.redirect(`/invoices/${invoice.id}`);
});

router.post('/:id/paid', verifyCsrf, (req, res) => {
  const invoice = db.prepare('SELECT * FROM invoices WHERE id = ?').get(req.params.id);
  if (!invoice) return res.status(404).render('error', { message: 'Invoice not found.' });
  ...
  db.prepare("UPDATE invoices SET status = 'paid', paid_at = datetime('now'), updated_at = datetime('now') WHERE id = ?").run(
    invoice.id
  );
...
router.get('/:id/print', (req, res) => {
  ...
  const items = db.prepare('SELECT * FROM invoice_items WHERE invoice_id = ? ORDER BY id').all(invoice.id);
  const total = totalFor(invoice.id);
